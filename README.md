...
`add_edge()` で辺を設定し、`solve()` で全点対最短距離を計算します。
...
- 非整合: 辺追加後で、内部の距離行列の再計算が必要な状態

#### 使用上の注意

- `add_edge(..., update_dists=False)` により緩和を行った場合、距離行列の整合性が崩れます。
- 距離行列が非整合の場合、以下の操作は行えず、操作を行うと `RuntimeError` が発生します。
  - `dist` の参照
...
一度 `solve()` されている、あるいは辺が存在しない初期状態であれば、辺の追加に対して `O(n^2)` で距離行列を更新できます。
